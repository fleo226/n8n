...
/** Seed open + closed requests in the shared team project for inbox GETs. */
...
describe('GET /workflow-review-requests', () => {
	...
	test('returns empty list for project viewer', async () => {
		...
		const response = await viewerAgent.get('/workflow-review-requests').expect(200);
	...
	test('returns 403 when license is disabled', async () => {
		...
		await ownerAgent.get('/workflow-review-requests').expect(403);
	});
	...
	test('hides reviews from projects the member cannot access', async () => {
		...
		const memberResponse = await memberAgent.get('/workflow-review-requests').expect(200);
		expect(memberResponse.body.data.data).toEqual([]);

		const ownerResponse = await ownerAgent.get('/workflow-review-requests').expect(200);
