...
export class WorkflowPublicationApplier {
	 ...
	 * version being published (`newVersion` — the history row for the workflow's
	 * current `activeVersionId`, null if that row no longer exists or the workflow
	 * is unpublished) and the currently published version (`oldVersion`, null on a
	 * first publication). The workflow is loaded independently of the
	 * published-version mapping so a first publication (no mapping row yet) still
	 * resolves it.
	 */
	private async resolveVersions(record: WorkflowPublicationOutbox): Promise<{
		...
		const newVersion = workflow?.activeVersionId
			? await this.workflowHistoryRepository.findOneBy({ versionId: workflow.activeVersionId })
			: null;
