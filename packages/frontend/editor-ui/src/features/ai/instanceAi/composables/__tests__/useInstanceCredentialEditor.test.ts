...
describe('useInstanceCredentialEditor', () => {
	...
	it('reports the newly created credential once the modal closes', async () => {
		...
		openModal();
		await nextTick();
		// The credential list refreshes while the modal is open.
	...
	it('does not fire when a modal stacks on top of the credential modal', async () => {
		...
		openModal();
		await nextTick();
		stackModal(); // credential modal is no longer topmost but still open
		await nextTick();

		expect(onClosed).not.toHaveBeenCalled();

		// Unwinding the stacked modal still keeps the credential modal open.
