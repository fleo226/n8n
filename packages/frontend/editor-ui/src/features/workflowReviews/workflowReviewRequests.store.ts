...
import {
	...
	fetchWorkflowReviewRequestsSummary,
} from './workflowReviewRequests.api';
...
export const useWorkflowReviewRequestsStore = defineStore('workflowReviewRequests', () => {
	const rootStore = useRootStore();

	const probeSettled = ref(false);
	const hasAnyReviews = ref(false);
	const items = ref<WorkflowReviewRequestSummaryDto[]>([]);
	...
	/** Monotonic id so slower responses cannot overwrite newer list queries. */
	...
	const isEmpty = computed(() => showSidebar.value && !loading.value && items.value.length === 0);

	function applyListResponse(
		response: ListWorkflowReviewRequestResponse,
	...
	async function requestList(cursor?: string): Promise<ListWorkflowReviewRequestResponse> {
		return await fetchWorkflowReviewRequests(rootStore.restApiContext, {
	...
	async function probeInbox() {
		...
		try {
			const summary = await fetchWorkflowReviewRequestsSummary(rootStore.restApiContext);
	...
	function setActiveState(state: WorkflowReviewRequestState) {
